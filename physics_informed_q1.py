...
Original file is located at
    https://colab.research.google.com/drive/1Un_Kx_pjZaD3YtRn7Cxhy07cMyx1437b
"""
...
class PhysicsInformedBarModel_q1:
    """A class used for the definition of Physics Informed Models for one dimensional bars."""

    def __init__(self, E, A, L, u0, load):
        """Construct a PhysicsInformedBarmodel_Q1"""
        self.E = E
        self.A = A
        self.L = L
        self.x = utilities_q1.generate_grid_1d(L)
        self.u0 = u0
        self.load = load
        self.model = utilities_q1.build_model(1,[41,37],1)
    ...
    def costFunction(self, x, u_pred):
        ...
        differential_equation_loss = self.E(x) * self.A(x) * utilities_q1.get_derivative(u_pred, x, 1) - self.load(x)
        ...
        # Sum over Neuman boundary condition loss
        boundary_condition_loss += (self.E(x) * self.A(x)[-1] * utilities_q1.get_derivative(u_pred, x, 1)[-1] - self.load(x)) ** 2
